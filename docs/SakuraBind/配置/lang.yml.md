...
send-back:
  ...
  database-all: '&7你的{0}个遗失物品已全部放入你的&a暂存箱, 请输入 &6''/sakurabind getLost'' &a领取'
...
bind-item:
  ...
  unbind-success: '&a全部解绑成功'
  unbind-success-remain: '&a成功解绑 {0} 个物品'
  bind-failure: '&6绑定失败'
  bind-success: '&a全部绑定成功'
  bind-success-remain: '&a成功绑定 {0} 个物品'
  no-amount: '&6物品数量不足'
addons:
